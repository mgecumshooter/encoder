# ENCODER/DECODER
Very basic Vigener encoder writen in c++

## WARNING

Currently, there is no releases, nor just compiled binaries. Build it on your own risk, or wait until first release (<= 20.12.2025)

### USAGE:
  run compiled binary in your terminal, all others intuitively clear in this CLI.
  just going to inform some warnings/tips:
  
    - dont try to put unexepcted input (characters, when expected numbers; special characters in keys, etc.)
    - put letters **only** from alphabet you selected
    - currently, keys dont support special characters (all except alphabet letters), but it will be fixed in future
    - GUI coming in future
